	...
	public static void calculateRanking(ArrayList<HockeyPlayer> playerList,
		...
		for (HockeyPlayer player : playerList) {
			...
			if (selectedStats[0] = true) {
				rank += ((player.getGoals() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgGoals()) / leagueStats.getStdGoals();
			}
			if (selectedStats[1] = true) {
				rank += ((player.getAssists() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgAssists()) / leagueStats.getStdAssists();
			}
			if (selectedStats[2] = true) {
				rank += ((player.getPlusminus() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgPlusminus()) / leagueStats.getStdPlusminus();
			}
			if (selectedStats[3] = true) {
				rank += ((player.getPims() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgPims()) / leagueStats.getStdPims();
			}
			if (selectedStats[4] = true) {
				rank += ((player.getShots() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgShots()) / leagueStats.getStdShots();
			}
			if (selectedStats[5] = true) {
				rank += ((player.getHits() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgHits()) / leagueStats.getStdHits();
			}
			if (selectedStats[6] = true) {
				rank += ((player.getBlocks() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgBlocks()) / leagueStats.getStdBlocks();
			}
			if (selectedStats[7] = true) {
				rank += ((player.getTakeaways() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgTakeaways()) / leagueStats.getStdTakeaways();
			}
			if (selectedStats[8] = true) {
				rank += ((player.getPowerplay() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgPowerplay()) / leagueStats.getStdPowerplay();
			}
			if (selectedStats[9] = true) {
				rank += ((player.getShorthanded() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgShorthanded())
						/ leagueStats.getStdShorthanded();
			}
			if (selectedStats[10] = true) {
				rank += ((player.getGameWinning() * 1.0)
						/ (player.getGamesPlayed() * 1.0) - leagueStats
							.getAvgGameWinning())
						/ leagueStats.getStdGameWinning();
			}
